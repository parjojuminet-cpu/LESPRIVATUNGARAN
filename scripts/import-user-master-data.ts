...
function generateUsername(fullName: string): string {
  ...
  // Choose nickname if parents had (e.g. Dila or Fina)
  ...
  // Use the first word as the primary username
  return parts[0];
}
...
async function migrate() {
  ...
  // To assign stable and clean IDs
  let tutorCounter = 1;
  ...
  // Unique usernames pool to avoid conflicts
  ...
  for (const item of rawInput) {
    ...
    if (!tutorsMap.has(tutorName)) {
      ...
      const password = '123'; // Simple but robust standard password

      tutorsMap.set(tutorName, {
        id: tutorId,
        name: tutorName,
        gender: 'Perempuan', // Standard default for female team names
        address: address || 'Ungaran',
        wa: '0812-3456-7890', // Default WA for tutors
      ...
      usersList.push({
        ...
        passwordHash: password, // Store password plainly as requested or hash if supported, UI accepts both
        name: tutorName,
        role: 'TENTOR',
        tutorId: tutorId,
        status: 'Aktif',
    ...
    if (!studentsMap.has(studentKey)) {
      ...
      let gradeLevel: 'PAUD' | 'TK Kecil' | 'TK Besar' | 'SD' | 'SMP' | 'SMA' = 'SD';
      ...
      studentsMap.set(studentKey, {
        id: studentId,
        name: studentName,
        gender: 'Laki-laki', // Default, editable
        grade: gradeLevel,
        className: gradeRaw,
    ...
    schedulesList.push({
      ...
      dayOfWeek: 'Senin', // Default to Monday, easy to reschedule in app
  ...
  // Build final collections arrays
  const students = Array.from(studentsMap.values());
  const tutors = Array.from(tutorsMap.values());
  const parents = Array.from(parentsMap.values());

  const subjects = [
    { id: 'sub_001', name: 'Calistung', grade: 'TK Besar' as const },
    { id: 'sub_002', name: 'Matematika', grade: 'SD' as const },
    { id: 'sub_003', name: 'English', grade: 'SD' as const },
    { id: 'sub_004', name: 'Ngaji Iqro', grade: 'SD' as const },
    { id: 'sub_005', name: 'Hafalan', grade: 'SD' as const },
    { id: 'sub_006', name: 'Ngaji Qiroati', grade: 'SD' as const }
  ];
  ...
  console.log('Writing main_database to Cloud Firestore...');
  const docRef = doc(db, 'erp_store', 'main_database');
  await setDoc(docRef, databasePayload);

  console.log('=========================================');
  console.log('🎉 SUCCESS: ERP Database has been fully populated!');
  console.log(`Students imported: ${students.length}`);
  console.log(`Tutors imported: ${tutors.length}`);
  console.log(`Parents imported: ${parents.length}`);
  console.log(`Schedules generated: ${schedulesList.length}`);
  console.log(`User Accounts Created: ${usersList.length}`);
  console.log('=========================================');

  console.log('LOGIN DATA FOR TUTORS:');
  console.log('-----------------------------------------');
  usersList.forEach(u => {
    if (u.role === 'TENTOR') {
      console.log(`Tutor: ${u.name.padEnd(25)} | Username: ${u.username.padEnd(12)} | Password: ${u.passwordHash}`);
    } else {
      console.log(`Admin: ${u.name.padEnd(25)} | Username: ${u.username.padEnd(12)} | Password: ${u.passwordHash}`);
    }
  });
  console.log('-----------------------------------------');
  process.exit(0);
