...
export default function App() {
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      if (resStudents?.length) setStudents(resStudents);
      if (resTutors?.length) setTutors(resTutors);
      if (resParents?.length) setParents(resParents);
      if (resSubjects?.length) setSubjects(resSubjects);
      if (resAreas?.length) setWorkingAreas(resAreas);
      if (resSchedules?.length) setSchedules(resSchedules);
      if (resAttendances?.length) setAttendances(resAttendances);
      if (resInvoices?.length) setInvoices(resInvoices);
      if (resFinance?.length) setFinance(resFinance);
      if (resSalaries?.length) setSalaries(resSalaries);
      if (resApprovals?.length) setApprovals(resApprovals);
      if (resModules?.length) setModules(resModules);
      if (resSettings?.length) setSettings(resSettings);
      if (resAuditLogs?.length) setAuditLogs(resAuditLogs);
