...
export function sanitizeErpDatabase(db: Partial<ErpDatabaseJson>): ErpDatabaseJson {
  ...
  if (!users.some(u => u.username === 'admin')) {
    users = [...DEFAULT_JSON_USERS, ...users];
  }

  const cleanTutors = (db.tutors || []).filter(t => !DUMMY_IDS.has(t.id) && !t.name?.includes('Budi Santoso') && !t.name?.includes('Rina Wijaya'));
  const cleanStudents = (db.students || []).filter(s => !DUMMY_IDS.has(s.id) && !s.name?.includes('Ananda Rizky') && !s.name?.includes('Bintang Perkasa') && !s.name?.includes('Citra Kirana'));

  return {
    users,
    students: cleanStudents,
    tutors: cleanTutors,
    parents: (db.parents || []).filter(p => !DUMMY_IDS.has(p.id)),
    subjects: (db.subjects || []).filter(s => !DUMMY_IDS.has(s.id)),
    workingAreas: (db.workingAreas || []).filter(w => !DUMMY_IDS.has(w.id)),
    schedules: (db.schedules || []).filter(s => !DUMMY_IDS.has(s.id)),
