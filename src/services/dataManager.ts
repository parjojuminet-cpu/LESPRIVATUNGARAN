...
export async function saveStudentData(student: Partial<Student> & { id?: string }): Promise<ErpDatabaseJson> {
  ...
  const fullStudent: Student = {
    ...
    totalPackageSessions: Number(student.totalPackageSessions || 12),
    remainingSessions: Number(student.remainingSessions || 12),
...
// Tutor CRUD
export async function saveTutorData(tutor: Partial<Tutor> & { id?: string }): Promise<ErpDatabaseJson> {
  const isEdit = !!tutor.id;
  const newId = tutor.id || `ttr_${Date.now()}`;
  ...
  return persistDatabaseUpdate(db => {
    ...
    const updatedTutors = exists
      ...
      : [fullTutor, ...db.tutors];
    return { ...db, tutors: updatedTutors };
  });
}
...
export async function payInvoiceData(invoiceId: string, paymentMethod: string): Promise<ErpDatabaseJson> {
  try {
  ...
  return persistDatabaseUpdate(db => {
    ...
    const updatedInvoices = db.invoices.map(i => i.id === invoiceId ? { ...i, status: 'Lunas' as const, amountPaid: i.amount } : i);
    ...
    if (targetInvoice) {
      ...
      updatedFinance = [newFinance, ...db.finance];
    }
