...
export const Header: React.FC<HeaderProps> = ({
  ...
  const getPageMeta = (tab: string) => {
    switch (tab) {
      ...
      case 'schedule':
        return { label: 'Penjadwalan Sesi', icon: Calendar, color: 'text-sky-600 bg-sky-50 border-sky-200' };
      case 'attendance':
  ...
  return (
    <header className="bg-white/95 backdrop-blur-md border-b border-slate-200/90 sticky top-0 z-30 px-3.5 sm:px-6 lg:px-8 py-2.5 shadow-xs transition-all">
      <div className="flex items-center justify-between gap-3 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          {onToggleMobileMenu && (
            <button
              ...
              className="lg:hidden p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 transition-all cursor-pointer border border-slate-200 shadow-2xs active:scale-95"
          ...
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-indigo-600 via-indigo-600 to-violet-700 text-white flex items-center justify-center font-black text-sm sm:text-base shadow-md shadow-indigo-200 shrink-0 border border-indigo-400/30">
              ERP
            </div>
            <div>
              <h1 className="text-sm sm:text-base font-black text-slate-900 tracking-tight leading-none flex items-center gap-2">
                ERP Bimbel Privat
                <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200 hidden xs:inline-flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> Live
                </span>
              </h1>
              <p className="text-[11px] text-slate-500 mt-0.5 line-clamp-1 font-medium hidden sm:block">Sistem Presensi, Penjadwalan & Keuangan</p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {currentUser.role !== 'TENTOR' && (
            <div className="flex items-center gap-1 bg-indigo-50/80 border border-indigo-100 p-1 rounded-xl">
              ...
              <select
                ...
                className="text-xs bg-white text-slate-800 font-extrabold px-2 py-1 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 cursor-pointer max-w-[110px] sm:max-w-none shadow-2xs"
              >
                {users.map((u) => (
          ...
          <div className="flex items-center gap-2 pl-2 border-l border-slate-200">
            ...
            <button
              onClick={onLogout}
              title="Keluar dari ERP & Otomatis Simpan Seluruh Data ke Google Sheets"
              className="flex items-center gap-1.5 text-xs bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 px-2.5 sm:px-3 py-1.5 rounded-xl transition-all cursor-pointer font-bold shadow-2xs active:scale-95"
            >
