            ...
            onChange={(e) => {
              ...
              if (selectedUsername) {
                ...
                if (target) {
                  ...
                  setPassword(target.passwordHash || 'tentor123');
                  setErrorMsg(null);
                }
              }
            }}
            className="w-full bg-white border border-slate-300 rounded-xl p-2 font-medium text-slate-800 text-xs focus:ring-2 focus:ring-indigo-500 focus:outline-none"
          >
            <option value="">-- Klik untuk Pilih & Isi Otomatis --</option>
            <optgroup label="Manajemen & Admin">
              {users.filter(u => u.role !== 'TENTOR').map(u => (
                <option key={u.id || u.username} value={u.username}>
                  [{u.role}] {u.name} ({u.username})
                </option>
              ))}
            </optgroup>
            <optgroup label="Akun Tentor">
              {users.filter(u => u.role === 'TENTOR').map(u => (
                <option key={u.id || u.username} value={u.username}>
                  [TENTOR] {u.name} ({u.username} / {u.passwordHash || 'tentor123'})
                </option>
              ))}
            </optgroup>
          </select>
