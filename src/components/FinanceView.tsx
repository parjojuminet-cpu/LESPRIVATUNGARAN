...
import { saveInvoiceData, payInvoiceData, persistDatabaseUpdate } from '../services/dataManager';
...
export const FinanceView: React.FC<FinanceViewProps> = ({
  ...
  const displaySalaries = tutors.map(tut => {
    const existingSal = salaries.find(s => s.tutorId === tut.id);
    
    // Real-time calculation from attendances marked 'Hadir'
    ...
    const totalAttendanceRate = (existingSal?.totalAttendanceRate && existingSal.totalAttendanceRate > 0)
      ? existingSal.totalAttendanceRate
      : totalAttendanceRateCalculated;

    const bonus = existingSal?.bonus || 0;
    const compensation = existingSal?.cancellationCompensation || 0;
    const deductions = existingSal?.deductions || 0;
    const totalSalary = (existingSal?.totalSalary && existingSal.totalSalary > 0)
      ? existingSal.totalSalary
      : (totalAttendanceRate + bonus + compensation - deductions);

    const paymentStatus = existingSal?.paymentStatus || (tutHadirAttendances.length > 0 ? 'Pending' : 'N/A');

    return {
      id: existingSal?.id || `sal_${tut.id}`,
      monthYear: existingSal?.monthYear || currentMonthYear,
      tutorId: tut.id,
      tutorName: tut.name,
      tutorWa: tut.wa,
  ...
  }).filter(s => s.totalSessions > 0 || s.totalSalary > 0 || salaries.some(sal => sal.tutorId === s.tutorId));
                      ...
                      <td className="p-3.5 text-right space-x-1">
                        {sal.paymentStatus !== 'Paid' && (
                          <button
                            ...
                            className="px-3 py-1 bg-emerald-600 text-white font-bold rounded-lg text-[11px] hover:bg-emerald-700 cursor-pointer shadow-xs active:scale-95 transition-all"
                          >
                            Cairkan Gaji
                          </button>
                        )}
