...
import { Camera, CheckCircle, Clock, AlertTriangle, UserCheck, RefreshCw, Sparkles, FileText, Loader2, SwitchCamera } from 'lucide-react';
import { saveAttendanceData } from '../services/dataManager';
            ...
            {status === 'Hadir' && (
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
                <label className="block text-slate-800 font-bold flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full font-semibold border border-emerald-200">
                    {facingMode === 'user' ? 'Kamera Depan' : 'Kamera Belakang'}
                  </span>
                </label>

                {selfieUrl ? (
                  <div className="relative rounded-xl overflow-hidden border border-emerald-300 max-h-48 flex items-center justify-center bg-slate-900">
                    <img src={selfieUrl} alt="Foto bukti preview" className="w-full h-48 object-cover" />
                    <button
                      type="button"
                      onClick={() => setSelfieUrl('')}
                      className="absolute top-2 right-2 bg-slate-900/80 text-white text-[10px] font-bold px-2.5 py-1 rounded-lg backdrop-blur-xs hover:bg-rose-600 cursor-pointer transition-all flex items-center gap-1"
                    >
                      <RefreshCw className="w-3 h-3" /> Ambil Foto Ulang
                    </button>
                  </div>
                ) : isCameraActive ? (
                  <div className="relative rounded-xl overflow-hidden bg-black text-center">
                    <video ref={videoRef} autoPlay playsInline className="w-full h-48 object-cover" />
                    
                    <div className="absolute bottom-2 inset-x-2 flex items-center justify-between gap-2 bg-slate-950/80 p-2 rounded-xl backdrop-blur-xs">
                      <button
                        type="button"
                        onClick={() => startCamera(facingMode === 'user' ? 'environment' : 'user')}
                        className="bg-slate-800 hover:bg-slate-700 text-white font-bold px-3 py-1.5 rounded-lg text-[11px] flex items-center gap-1 cursor-pointer transition-all border border-slate-700"
                      ...
                      <button
                        ...
                        className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-extrabold px-4 py-1.5 rounded-lg text-xs shadow-md cursor-pointer flex items-center gap-1 transition-all active:scale-95"
                      >
                      ...
                      <button
                        type="button"
                        onClick={stopCamera}
                        className="bg-rose-600 hover:bg-rose-700 text-white font-bold px-2.5 py-1.5 rounded-lg text-[11px] cursor-pointer"
                      >
                        Tutup
                      </button>
                    </div>
